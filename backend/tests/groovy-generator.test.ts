import { generateGroovy } from '../src/core/groovy-generator';
import { MappingSpec } from '../src/core/types';

const baseSpec: MappingSpec = {
  id: 'test-001',
  name: 'Test Mapping',
  sourceFormat: 'JSON',
  targetFormat: 'JSON',
...
describe('groovy-generator', () => {
  test('generates a Groovy artifact', () => {
    const artifact = generateGroovy(baseSpec);
  ...
  test('CSV source generates rows.collect loop', () => {
    const spec: MappingSpec = { ...baseSpec, sourceFormat: 'CSV', rules: [{ id: 'r1', sourcePaths: ['MATNR'], targetPath: 'materialId' }] };
    const artifact = generateGroovy(spec);
    expect(artifact.content).toContain('rows.collect');
  });

  test('XML target generates MarkupBuilder', () => {
    const spec: MappingSpec = { ...baseSpec, targetFormat: 'XML', targetSchema: [{ path: 'Invoice.Id', name: 'Id', type: 'string', required: true }], rules: [{ id: 'r1', sourcePaths: ['invoiceId'], targetPath: 'Invoice.Id' }] };
    const artifact = generateGroovy(spec);
